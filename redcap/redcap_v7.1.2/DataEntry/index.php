...
if ($auto_inc_set)
{
	...
	{
		if (Records::recordExists($fetched)) {
...
$isRepeatingForm = ($isRepeatingFormOrEvent && isset($Proj->RepeatingFormsEvents[$_GET['event_id']][$_GET['page']]));
...
if (isset($_POST['submit-action']))
{
	...
	if ($_POST['submit-action'] != 'submit-btn-cancel' && $auto_inc_set && $_POST['hidden_edit_flag'] == '0') {
		$fetched = $_POST[$table_pk] = Records::addNewRecordToCache($_GET['event_id'], $fetched);
	...
	if ($_POST['submit-action'] != 'submit-btn-cancel' && $user_rights['lock_record'] <= 1) 
	{
		...
		$sql = "select l.username, l.timestamp, f.display_esignature from redcap_locking_data l 
				...
				where l.project_id = $project_id and l.record = '" . prep($fetched.$entry_num) . "' 
				and l.event_id = {$_GET['event_id']} and l.form_name = '" . prep($_GET['page']) . "'
	...
	if ($openDDP === null) checkReqFields($fetched);
	
	// Save state of 2nd save button
	$secondBtnNames = array('submit-btn-savecompresp', 'submit-btn-savecontinue', 'submit-btn-savenextform', 'submit-btn-savenextinstance');
	...
	switch ($_POST['submit-action'])
	{
		...
		case 'save-and-redirect':
			...
			elseif ($user_rights['record_delete'] && $_POST['submit-action'] == "submit-btn-deleteform")
			{
				...
				$sql = "select distinct field_name from redcap_data where project_id = $project_id
						and event_id = {$_GET['event_id']} and record = '".prep($fetched.$entry_num)."'
				...
				if ($surveys_enabled && isset($Proj->forms[$_GET['page']]['survey_id'])) 
				{
					$sql2 = "update redcap_surveys_participants p, redcap_surveys_response r
							...
							and r.record = '".prep($fetched.$entry_num)."' and p.event_id = {$_GET['event_id']} and r.instance = {$_GET['instance']}";
					db_query($sql2);
				...
				// Log the data change
				$log_event_id = Logging::logEvent($sql."; $sql2", "redcap_data", "UPDATE", $fetched.$entry_num, implode(",\n",$eraseFieldsLogging), "Delete all record data for single form",
			...
			if ($_POST['submit-action'] == "submit-btn-savecontinue")
			{
				redirect(PAGE_FULL . "?pid=$project_id&page={$_GET['page']}&id=$fetched_msg&event_id={$_GET['event_id']}&instance={$_GET['instance']}"
					...
					. (empty($openDDP) ? '' : "&openDDP=1") . $dq_error_ruleids . $serverside_error_fields . "&msg=edit");
			}
			// Redirect back to same page AND pop-up the Data Resolution Workflow dialog
			elseif ($dqresfld !== null)
			{
				redirect(PAGE_FULL . "?pid=$project_id&page={$_GET['page']}&id=$fetched_msg&event_id={$_GET['event_id']}" . ((isset($_GET['editresp']) && $_GET['editresp']) ? "&editresp=1" : "") . (empty($scrollTop) ? '' : "&scrollTop=$scrollTop") . "&fldfocus=$dqresfld&dqresfld=$dqresfld");
			}
		...
		case 'submit-btn-cancel':
			...
			redirect(APP_PATH_WEBROOT . "DataEntry/record_home.php?pid=$project_id&id=$fetched&arm=" . getArm() . "&msg=cancel");
...
if (isset($_GET['page'])) {
	if (isset($_GET['id'])) {
		...
		$group_id = (empty($Proj->groups)) ? null : Records::getRecordGroupId(PROJECT_ID, $fetched);
		if (!is_numeric($group_id)) $group_id = null;
		Hooks::call('redcap_data_entry_form_top', array(PROJECT_ID, ($hidden_edit ? $fetched : null), $_GET['page'], $_GET['event_id'], $group_id));
	} else {
...
print	RCView::div(array('style'=>'max-width:800px;margin:-13px 0 0;'),
				...
				RCView::div(array('id'=>'pdfExportDropdownDiv', 'style'=>'display:none;position:absolute;z-index:1000;'),
					RCView::ul(array('id'=>'pdfExportDropdown'),
						...
						(!(isset($_GET['id']) && $hidden_edit && $user_rights['data_export_tool'] > 0) ? '' :
							RCView::li(array(),
								RCView::a(array('href'=>'javascript:;', 'onclick'=>"window.location.href = app_path_webroot+'PDF/index.php?pid='+pid+'&page={$_GET['page']}&id={$_GET['id']}{$entry_num}&event_id={$_GET['event_id']}'+(getParameterByName('instance')==''?'':'&instance='+getParameterByName('instance'));"),
									RCView::img(array('src'=>'pdf.gif')) .
				...
				<tr>
					...
					<td class="data">
						<select id="record_select1" class="x-form-text x-form-field notranslate" style="max-width:350px;"
							onchange="if(this.value.length>0){window.location.href=app_path_webroot+page+'?pid='+pid+'&page=<?php echo $_GET['page'] ?>&id='+this.value+addGoogTrans();}">
		...
		$page_instructions =   "<br><span class='yellow' style='padding-right:15px;'>
								...
								<p style='padding:25px 0 20px;color:#666;'>
									<button class='jqbutton' onclick=\"window.location.href=app_path_webroot+'DataEntry/record_home.php?pid=$project_id&page=&arm=$arm&id=$fetched';\">
										<img src='" . APP_PATH_IMAGES . "arrow_left.png'> {$lang['data_entry_55']} $table_pk_label <b>$fetched</b>
...
elseif (isset($_GET['id']))
{
	...
	if ($user_rights['forms'][$_GET['page']] == '2') {
		...
		// $elements[] = array('rr_type'=>'button', 'btnclass'=>'btn btn-primary disabled', 'value'=>$lang['data_entry_206'], 'disabled'=>'disabled');
		...
		if ($isRepeatingForm) {			
			...
			$instances = RepeatInstance::getRepeatFormInstanceList(addDDEending($_GET['id']), $_GET['event_id'], $_GET['page']);
			...
			$next_instance_button_li = '<li><a href="javascript:;" id="submit-btn-savenextinstance" onclick="dataEntrySubmit(\'submit-btn-savenextinstance\');return false;">'.$next_instance_button_text.'</a></li>';
			$next_instance_button = '<button class="btn btn-primary" id="submit-btn-savenextinstance" name="submit-btn-savenextinstance" onclick="dataEntrySubmit(this);return false;" style="margin-bottom:2px;font-size:13px !important;padding:6px 8px;" tabindex="'.($saveButtonTabIndex+1).'">'.$next_instance_button_text.'</button>';
		}

		// If user is on last form, don't show the button "Save and go to Next Form"
		$next_form_button = $next_form_button_li = '';
		if ($_GET['page'] != $last_form) {
			$next_form_button_li = '<li><a href="javascript:;" id="submit-btn-savenextform" onclick="dataEntrySubmit(\'submit-btn-savenextform\');return false;">'.$lang['data_entry_210'].'</a></li>';
			$next_form_button = '<button class="btn btn-primary" id="submit-btn-savenextform" name="submit-btn-savenextform" onclick="dataEntrySubmit(this);return false;" style="margin-bottom:2px;font-size:13px !important;padding:6px 8px;" tabindex="'.($saveButtonTabIndex+1).'">'.$lang['data_entry_210'].'</button>';
		}
		...
		if (isset($Proj->forms[$_GET['page']]['survey_id']) && $user_rights['forms'][$_GET['page']] == '3')
		{
			...
			if (isset($_GET['editresp'])) {
				...
				$sql = "select r.first_submit_time from redcap_surveys_participants p, redcap_surveys_response r
						where r.participant_id = p.participant_id and r.record = '".prep(addDDEending($_GET['id']))."'
			...
			$comp_resp_button_li = '<li><a href="javascript:;" id="submit-btn-savecompresp" onclick="dataEntrySubmit(\'submit-btn-savecompresp\');return false;">'.$lang['data_entry_212'].'</a></li>';
			$comp_resp_button = '<button class="btn btn-primary" id="submit-btn-savecompresp" name="submit-btn-savecompresp" onclick="dataEntrySubmit(this);return false;" style="margin-bottom:2px;font-size:13px !important;padding:6px 8px;" tabindex="'.($saveButtonTabIndex+1).'">'.$lang['data_entry_212'].'</button>';
		}
		
		// Set Save and Stay options
		$stay_form_button_li = '<li><a href="javascript:;" id="submit-btn-savecontinue" onclick="dataEntrySubmit(\'submit-btn-savecontinue\');return false;">'.$lang['data_entry_292'].'</a></li>';
		$stay_form_button = '<button class="btn btn-primary" id="submit-btn-savecontinue" name="submit-btn-savecontinue" onclick="dataEntrySubmit(this);return false;" style="margin-bottom:2px;font-size:13px !important;padding:6px 8px;" tabindex="'.($saveButtonTabIndex+1).'">'.$lang['data_entry_292'].'</button>';
				
		// Set the drop-down save button options
		$dropdownSaveBtn = '';
		$only_has_stay_form_option = ($next_instance_button_li.$next_form_button_li.$comp_resp_button_li == '');
		if (!$only_has_stay_form_option) {
			$dropdownSaveBtn = '<button id="submit-btn-dropdown" title="'.cleanHtml2($lang['data_entry_287']).'" class="btn btn-primary btn-savedropdown dropdown-toggle" style="margin-bottom:2px;font-size:13px !important;padding:6px 8px;" tabindex="'.($saveButtonTabIndex+2).'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" onclick="openSaveBtnDropDown(this,event);return false;">
		...
		if ($secondBtnState == null && !$only_has_stay_form_option) {
			$secondBtn = '<button id="submit-btn-placeholder" data-trigger="click" data-toggle="popover" data-placement="top" data-content="'.cleanHtml2($lang['data_entry_290']).'" title="<b>'.cleanHtml2($lang['data_entry_291']).'</b>" class="btn btn-primary btn-saveand" onclick="return false;" style="margin-bottom:2px;font-size:13px !important;padding:6px 8px;" tabindex="'.($saveButtonTabIndex+1).'">'.$lang['data_entry_289'].'</button>';
		...
		$elements[] = array('rr_type'=>'static', 'name'=>'__SUBMITBUTTONS__', 'label'=>'',
			'value'=>'<div id="__SUBMITBUTTONS__-div" style="margin:5px 0;">
						<button class="btn btn-primary" id="submit-btn-saverecord" name="submit-btn-saverecord" onclick="dataEntrySubmit(this);return false;" style="margin-bottom:2px;font-size:13px !important;padding:6px 8px;" tabindex="'.$saveButtonTabIndex.'">'.$lang['data_entry_288'].'</button>&nbsp;
						<div class="btn-group nowrap">'.$secondBtn . $dropdownSaveBtn.'</div>
						<button class="btn btn-defaultrc btn-sm" style="display:block;margin-top:15px;font-size:13px;" name="submit-btn-cancel" onclick="dataEntrySubmit(this);return false;" style="color:#000;font-size:13px;" tabindex="'.($saveButtonTabIndex+3).'"/>'.$lang['data_entry_207'].'</button>
					</div>');
		//$elements[] = array('rr_type'=>'static', 'value'=>'<button class="btn btn-defaultrc btn-sm" name="submit-btn-cancel" onclick="dataEntrySubmit(this);return false;" style="color:#000;font-size:13px;" tabindex="'.($saveButtonTabIndex+5).'"/>'.$lang['data_entry_207'].'</button>');
		...
		if ($hidden_edit && $user_rights['record_delete'])
		{
			...
			if ($wasRecordRandomized) {
				...
				if ($formContainsRandFields) {
					$delFormRandOnclick = "simpleDialog('".cleanHtml($lang['data_entry_266'])."','".cleanHtml($lang['global_03'])."');return false;";
				}
			}
			...
			if (isset($Proj->forms[$_GET['page']]['survey_id']) && $user_rights['forms'][$_GET['page']] == '3' && isset($_GET['editresp'])) {
				$delFormAlertMsg .= RCView::div(array('style'=>'margin-top:15px;color:#800000;'), $lang['data_entry_241']);
			}
			...
			$delFormOnclick = "simpleDialog('".str_replace('"', '&quot;', cleanHtml("<div style='margin:10px 0;font-size:13px;'>$delFormAlertMsg</div>"))."','".str_replace('"', '&quot;', cleanHtml("{$lang['data_entry_237']} \"{$_GET['id']}\"{$lang['questionmark']}"))."',null,600,null,'".cleanHtml($lang['global_53'])."',function(){ dataEntrySubmit( document.getElementsByName('submit-btn-deleteform')[0] );return false; },'".cleanHtml($lang['data_entry_234'])."');return false;";
			...
			'value'=>'<div id="__DELETEBUTTONS__-div" style="padding:10px 0 5px;">
						<button class="btn btn-defaultrc btn-xs" name="submit-btn-deleteform" onclick="'.$delFormOnclick.'" style="color:#C00000;margin:3px 0 1px;padding:2px 8px;" tabindex="'.($saveButtonTabIndex+8).'"/>'.$lang['data_entry_234'].'</button><br>
	...
	*/
	...
	$datasql = "select field_name, value, if (instance is null,1,instance) as instance 
				...
				and event_id = {$_GET['event_id']} and record = '".prep($this_record)."' and field_name in ('__GROUPID__', ";
		...
		if (db_num_rows($q) > 0)
		{
			...
			if (db_num_rows($q) > 0) {
				...
				$sql = "INSERT INTO redcap_data (project_id, event_id, record, field_name, value) 
						VALUES ($project_id, {$_GET['event_id']}, '".prep($this_record)."', '__GROUPID__', '{$element_data['__GROUPID__']}')";
				db_query($sql);
			}
		}
	}
	...
	if ($isIE && vIE() <= 9) callJSfile('html5placeholder.js');
	...
	if ($longitudinal) {
		print Form::addHiddenFieldsOtherEvents();
	}
