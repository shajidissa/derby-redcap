...
if (isset($_REQUEST['action']))
{
	switch ($_REQUEST['action'])
	{
		...
		case "adddates":
			...
			if (!isset($_GET['display_only'])) {
				...
				if ($_GET['newid']) {
					...
					$sql = "insert into redcap_data (project_id, event_id, record, field_name, value) values ($project_id, $firstEvent, '".prep($_GET['idnumber'])."', '$table_pk', '".prep($_GET['idnumber'])."')";
					if (db_query($sql)) {
						//Logging
						Logging::logEvent($sql,"redcap_data","INSERT",$_GET['idnumber'],"$table_pk = '".prep($_GET['idnumber'])."'","Create record");
					}
					//If user is in DAG, add group_id for the new record also (for ALL events)
					if ($user_rights['group_id'] != "") {
						$sql = "insert into redcap_data (project_id, event_id, record, field_name, value) values ($project_id, $firstEvent, '".prep($_GET['idnumber'])."', '__GROUPID__', '{$user_rights['group_id']}')";
						db_query($sql);
					}
			...
			if (PAGE != "ProjectGeneral/print_page.php") {
				print  "	<div style='text-align:right;font-weight:normal;'>
								<img src='".APP_PATH_IMAGES."pencil.png'> <a href='".APP_PATH_WEBROOT."Calendar/scheduling.php?pid=$project_id&record=".htmlspecialchars(cleanHtml(removeDDEending($_GET['idnumber'])), ENT_QUOTES)."&arm=".getArm()."' style='text-decoration:underline;'>{$lang['global_27']}</a>
								&nbsp;
								<img src='".APP_PATH_IMAGES."printer.png'> <a href='javascript:;' onclick=\"window.open('".APP_PATH_WEBROOT."ProjectGeneral/print_page.php?pid=$project_id&action=adddates&schedule&display_only&idnumber=".htmlspecialchars(cleanHtml(removeDDEending($_GET['idnumber'])), ENT_QUOTES)."&arm=".getArm()."','myWin','width=850, height=800, toolbar=0, menubar=1, location=0, status=0, scrollbars=1, resizable=1');\" style='text-decoration:underline;'>{$lang['scheduling_35']}</a>
							</div>";
			}
			print  "		</td>
					<tr>
		...
		case 'generate_sched':
			...
			print  "<p>";
			print  "<input type='button' value='".cleanHtml($lang['scheduling_03'])."' id='createbtn' onclick=\"createSched('".htmlspecialchars(cleanHtml(removeDDEending($_GET['idnumber'])), ENT_QUOTES)."',{$_GET['newid']});\">&nbsp;
					<input type='button' id='cancelbtn' value='".cleanHtml($lang['global_53'])."' onclick=\"$('#table').html('');\">
					&nbsp;&nbsp;
		...
		case 'edit_single':
			...
			db_query($sql1);
			//LOGGING for single
			Logging::logEvent($sql1,"redcap_events_calendar","MANAGE",$_GET['cal_id'],calLogChange($_GET['cal_id']),"Edit calendar event");
...
while ($row = db_fetch_assoc($q)) {
	...
	if (isset($_GET['cal_id']) && $_GET['cal_id'] == $row['cal_id']) {
		print  "<tr id='row_{$row['cal_id']}' evstat='{$row['event_status']}'>
					<td class='data nowrap' style='text-align:center;color:#777;' id='sched_frow'>
						<input type='button' id='btn_{$row['cal_id']}' value='".cleanHtml($lang['designate_forms_13'])."' style='color:#000;font-size:11px;' onclick=\"saveEditCalEv({$row['cal_id']},'".htmlspecialchars(rawurldecode(urldecode($_GET['record'])), ENT_QUOTES)."',$arm)\">
					</td>
	...
	// Regular row to display
		...
		$notes_vis = RCView::escape(nl2br($row['notes']));
		$notes_invis = "";
...
print  "<div id='new_ad_hoc'>
			<div style='color:green;font-size:12px;padding:15px 0 0;'>
				...
				<input type='button' id='btn_newCalEv' value=' ".cleanHtml($lang['design_171'])." ' onclick=\"
