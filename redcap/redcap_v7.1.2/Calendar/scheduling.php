...
renderPageTitle("<table cellpadding=0 cellspacing=0 width='100%'><tr>
				 <td valign='top'>
					<img src='".APP_PATH_IMAGES."calendar_plus.png'> {$lang['global_25']}
				 </td>
...
if (!isset($_GET['record'])) {
				...
				<td style='padding-right:100px;'>
					" . ($user_rights['record_create'] ? "<span>" : "<span style='display:none;'>" ) . "
						...
						<input type='text' maxlength='50' class='x-form-text x-form-field' size='15' name='idnumber2' id='idnumber2' $text_val_string
							onkeyup=\"if (this.value.length > 0) { $('#idnumber').val(''); if ($('select#arm').length) { $('select#arm').val(''); } }\" value='".cleanHtml($new_record_name)."' hiddenval='".cleanHtml($new_record_name)."' $new_record_name_disable>
						</span>
	...
	foreach ($data as $this_record=>$eattr) {
		...
		foreach ($eattr as $this_event_id=>$fattr) {
			// Skip the repeat_instances sub-array
...
if (isset($_GET['record']))
{
	...
	if ($user_rights['group_id'] != "") {
		$group_sql  = "and c.record in (" . pre_query("select record from redcap_data where project_id = $project_id and field_name = '__GROUPID__'
	...
	$sql = "select distinct c.record, c.event_id, a.arm_num from redcap_events_calendar c, redcap_events_metadata m, redcap_events_arms a
			...
			$group_sql $dde_sql group by c.record order by abs(c.record), c.record, a.arm_num";
	$q = db_query($sql);
	while ($row = db_fetch_assoc($q)) {
...
?>
<div id='alert_text' title='<?php echo cleanHtml($lang['scheduling_86']) ?>' style='line-height:1.4em;display:none;padding:15px;'></div>
