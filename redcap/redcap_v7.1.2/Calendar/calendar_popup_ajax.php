	...
	switch ($_REQUEST['action']) {
		...
		case "edit_date":
			...
			if (!$q) {
				...
				$msg = $msg_saved_date;
				//LOGGING
				Logging::logEvent($sql,"redcap_events_calendar","MANAGE",$_GET['cal_id'],calLogChange($_GET['cal_id']),"Edit calendar event");
			}
			break;
		//Edit the visit time
		case "edit_time":
		...
		case "edit_notes":
			...
			if (db_query($sql)) {
				//LOGGING
				Logging::logEvent($sql,"redcap_events_calendar","MANAGE",$_POST['cal_id'],calLogChange($_POST['cal_id']),"Edit calendar event");
			}
			exit;
			break;
	}
