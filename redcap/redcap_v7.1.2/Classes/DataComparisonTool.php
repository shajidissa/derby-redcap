...
class DataComparisonTool
{
	...
	public function renderPage()
	{
			...
			foreach ($_POST as $this_field=>$this_value)
			{
				...
				if (isset($chkbox_fields[$this_field])) {
					foreach ($this_value as $this_key=>$this_value_sub) {
						...
						if ($this_value_sub) {
							$sql_all[] = $sql = "INSERT INTO redcap_data (project_id, event_id, record, field_name, value) VALUES ($project_id, $event_id3, '" . prep($record3). "', '$this_field', '" . prep($this_key). "')";
							if (db_query($sql)) {
				...
				// Regular non-checkbox fields
					...
					// Set sql
					$sql_all[] = $sql = "INSERT INTO redcap_data (project_id, event_id, record, field_name, value) VALUES ($project_id, $event_id3, '" . prep($record3). "', '$this_field', '" . prep($this_value). "')";
					if (db_query($sql)) {
		...
		print "<form action=\"{$_SERVER['REQUEST_URI']}\" method=\"post\" enctype=\"multipart/form-data\" name=\"datacomp\" target=\"_self\">";
		print "<table class='form_border'>
				<tr>
					...
					<td class='label_header' style='padding:10px;' rowspan='2'>
						<input style='font-weight:normal;' name='submit' type='submit' value='".cleanHtml($double_data_entry ? $lang['data_comp_tool_44'] : $lang['data_comp_tool_02'])."' onclick=\"
				...
				foreach (array_keys($evts) as $event_id1)
				{
					...
					if ($double_data_entry && !isset($_GET['merge'])) {
						...
						if (db_num_rows($q) > 0) {
							...
							$display_string .= "<form action='" . $_SERVER['REQUEST_URI'] . "&merge=1' method='post' name='merge{$loopNum}'>
					...
					$display_string .= "<div style='max-width:700px;'>
										<form action='".$_SERVER['REQUEST_URI']."&event_id=$event_id1&create_new=1' method='post' enctype='multipart/form-data' name='create_new' target='_self'>
						...
						if (!$show_merged_record) {
							if ($double_data_entry) {
								print "<p>{$lang['data_comp_tool_37']} " . substr($record1, 0, -3) . $lang['data_comp_tool_39'] . "</p>
									  <form action=\"".$_SERVER['REQUEST_URI']."&event_id=$event_id1&create_new=1\" method=\"post\" enctype=\"multipart/form-data\" target=\"_self\">
