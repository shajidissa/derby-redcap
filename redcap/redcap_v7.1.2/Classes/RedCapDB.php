...
class RedCapDB {
	...
	function __construct() {
		if (self::$db === null) {
			...
			// Set sql_mode
			$res =& self::$db->query("SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION'");
	...
	private function save($tablename, $fieldvals, $where=null) {
		...
		// not sure if the below fix is correct, (self::db::$last_query) doesn't work
	...
	function getAPITokens($orderProj=false, $project_id='') {
		...
		$sql = "SELECT u.api_token, u.username, p.project_id, p.app_title, u.api_export, u.api_import, u.mobile_app,
