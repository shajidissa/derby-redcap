...
class RestUtility
{
	public static function processRequestSuper($data)
	{
		...
		$token = prep($data['token']);
		
		# check user rights
		$sql = "
			SELECT username, super_user
			FROM redcap_user_information
			WHERE api_token = '".prep($token)."'
		...
		// Set username
		defined("USERID") or define("USERID", $row['username']);
	...
	**/
	public static function processRequest($tokenRequired=true)
	{
		...
		if ($tokenRequired)
		{
			if($data['content'] == 'project' && isset($data['data']) && $data['data'] != '')
			{
			...
			// Set username
			defined("USERID") or define("USERID", $userRights['username']);
