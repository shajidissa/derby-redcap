...
class PROMIS
{
	...
	public function promisApiRequest($url, $params=array(), $request_type="POST", $recursive=false, $bypassApiRegistration=false)
	{
		...
		if (!$bypassApiRegistration && $info['http_code'] == '411') {
	...
	public function buildPromisForm($assessment_id, $form_array, $form_oid, $participant_id=null, $response_id=null, $acknowledgementText="")
	{
		...
		foreach ($choicesArray as $attr) {
			if ($enhanced_choices == '1') {
				$choices .= RCView::input(array('class'=>'hide', 'type'=>'radio', 'value'=>$attr['value'], 'name'=>$attr['name']))
						 .  "<div class='enhancedchoice col-xs-12 col-sm-6'>"
						 .  "<label onclick='enhanceChoiceSelect(this)' for='{$attr['name']},value,{$attr['value']}'><span class='ec'>{$attr['label']}</span></label></div>";
			} else {
		...
		if ($enhanced_choices == '1') {
			$field_tr = RCView::tr(array('id'=>"$input_name-tr", 'sq_id'=>$input_name),
							RCView::td(array('class'=>'labelrc questionnum col-xs-1', 'valign'=>'top'),
								''
							) .
							RCView::td(array('class'=>'labelrc col-xs-11'),
								$html .
								$choices .
								// Reset link
			...
			$field_tr = RCView::tr(array('id'=>"$input_name-tr", 'sq_id'=>$input_name),
							...
							RCView::td(array('class'=>'labelrc col-xs-6'),
								$html
							) .
							RCView::td(array('class'=>'data col-xs-5'),
								$choices .
								// Reset link
								RCView::div(array('style'=>'text-align:right;line-height:10px;'),
				...
				function submit_cat() {
					if ($('#$html_form_name input:checked').length < 1) {
						...
						".($promis_skip_question
							? "simpleDialog(null,null,'cat_save_alert_dialog',null,\"$('#questiontable :input:first').trigger('focus');\",'".cleanHtml($lang['survey_563'])."',\"submit_cat_do();\",'".cleanHtml($lang['survey_561'])."');"
	...
	public static function saveParticipantResponses($assessment_id, $params, $form_name, $participant_id, $response_id=null, $end_survey=false, $skip_question=false)
	{
		...
		if ($end_survey) {
			...
			sendSurveyConfirmationEmail($Proj->forms[$form_name]['survey_id'], $_GET['event_id'], $fetched);
			sendEndSurveyEmails($Proj->forms[$form_name]['survey_id'], $_GET['event_id'], $participant_id, $fetched);
		}
