...
class MobileApp
{
	...
	public static function getAppDeviceDashboards($project_id)
        {
                ...
                $sql1 = "SELECT d.uuid AS uuid, d.nickname AS nickname, e.ts AS ts, l.event AS event, l.details AS details, 
						...
						FROM redcap_log_event e, redcap_mobile_app_log l
				...
				if (preg_match("/\.data\.csv$/", $row['doc_name'])) 
				{
					...
					foreach ($files as $subfile => $subrow) {
						if (!preg_match("/\.data\.csv$/", $subrow['doc_name']) && (strpos($subrow['doc_name'], $screen) !== false)) {
							...
							$url = APP_PATH_WEBROOT."DataEntry/file_download.php?pid=$project_id&doc_id_hash=".Files::docIdHash($subrow['doc_id'])."&id=".$subrow['doc_id'];
							if ($abbrev) {
								$subfiles[] = "<a style='font-size: 11px;' href='$url'>".$start[0]." ... ".$end[0]."</a>";
							} else {
								$subfiles[] = "<a style='font-size: 11px;' href='$url'>".$subrow['doc_name']."</a>";
							}
						}
					}
					...
					$url = APP_PATH_WEBROOT."DataEntry/file_download.php?pid=$project_id&doc_id_hash=".Files::docIdHash($row['doc_id'])."&id=".$row['doc_id'];
					$import_url = APP_PATH_WEBROOT."index.php";
					$import_data = "";
					if ($user_rights['data_import_tool'] == 1) {
						...
						{
							$gets2[] = RCView::escape($key."=".$val);
						}
						$import_data = RCView::button(array('class'=>'btn btn-defaultrc btn-xs', 'style'=>'margin:3px; 0;font-size:11px;', "onclick"=>"simpleDialog('".cleanHtml($lang['mobile_app_112'])."','".cleanHtml($lang['global_72'])."', null, 600, null, '".cleanHtml($lang['global_53'])."', function() { window.location.replace(\"".$import_url."?".implode("&", $gets2)."\"); }, '".cleanHtml($lang['global_72'])."');"), $lang['mobile_app_76']) . RCView::br();
					}
					$row_data[] = array(
									RCView::div(array('class'=>'wrap'), DateTimeRC::format_ts_from_ymd($row['stored_date'])),
									RCView::div(array('class'=>'wrap'), $row['doc_name']) .
									$import_data .
									RCView::button(array('class'=>'btn btn-primary btn-xs', 'style'=>'font-size:11px;', 'onclick'=>"window.open('$url','_blank');"), $lang['mobile_app_101']),
									$record_str,
	...
	// Obtain count of current users in a project that have init'd the project in the Mobile App
	public static function countUsersInitProject($project_id)
	{
		global $mobile_app_enabled;
		if (!$mobile_app_enabled) return 0;
		$sql = "select count(distinct(e.user)) from redcap_mobile_app_log a, redcap_log_event e, redcap_user_rights r 
				where a.event = 'INIT_PROJECT' and a.log_event_id = e.log_event_id and r.username = e.user 
				and r.project_id = a.project_id and a.project_id = $project_id";
		$q = db_query($sql);
		return db_result($q, 0);
	}	

	public static function getNickname($uuid, $project_id) {
		$sql = "SELECT nickname FROM redcap_mobile_app_devices WHERE (project_id = ".$project_id.") AND (uuid = '".prep($uuid)."')";
		$q = db_query($sql);
		while ($row = db_fetch_assoc($q)) {
	...
	public static function makeTableHeader($nickname, $uuid, $id, $project_id, $doc_id = null) {
		...
		if ($doc_id && ($doc_id !== ""))
		{
			...
			$link = RCView::button(array('class'=>'btn btn-defaultrc btn-xs', 'onclick'=>"window.open('$url','_blank');"),
						RCView::span(array('style'=>'vertical-align:middle', 'class'=>'glyphicon glyphicon-download-alt'), '') . 
		                ...
		                $title = RCView::div(array('style'=>'color:#800000;font-size:13px;padding:5px;'), $lang['mobile_app_96']);
                                ...
                                foreach ($table as $device)
                                {
                                        ...
                                        $row_data[] = array(RCView::div(array('class'=>'wrap'), $device['uuid']),
                                                        ...
                                                        "<td style='$border'>&nbsp;&nbsp;</td>" .
                                                        "<td style='$border'><button class='btn btn-primary btn-xs' id='".$device['device_id']."_button' style='$display2;' onclick='changeDeviceNickname(".$device['device_id'].", $(\"#".$device['device_id']."_nickname\").val(), \"".$device['uuid']."\");'>".$lang['mobile_app_100']."</button></td></tr>",
	...
	public static function displayInitPage()
	{
		...
		print	RCView::div(array('style'=>'text-align:center; max-width: 800px;'),
					...
					// VIDEO link
					RCView::span(array('class'=>'nowrap'),
						RCView::img(array('src'=>'video_small.png', 'style'=>'')) .
                                        ...
                                        RCView::tr(array('style' => 'border: 0;'),
                                                ...
                                                RCView::td(array('style' => 'border: 0; padding-left: 10px;'), 
													...
													RCView::div(array('style'=>'text-align:center;margin-top:20px;'),
														RCView::button(array('class'=>'btn btn-defaultrc btn-xs wrap', 'style'=>'max-width:150px;line-height: 12px;', 'onclick'=>"sendSingleEmail('".cleanHtml($userInfo['user_email'])."','".cleanHtml($userInfo['user_email'])."','".cleanHtml($lang['mobile_app_47'])."',trim($('#send_app_links_email_body').html()),true);"),
		...
		if (strlen($db->getAPIToken(USERID, PROJECT_ID)) == 32) {
			...
			if(ToDoList::checkIfRequestExist(PROJECT_ID, $ui_id, $todo_type) > 0){
				...
				$reqAPIBtn = RCView::button(array('class'=>'jqbuttonmed', 'onclick'=>"requestToken();"),
