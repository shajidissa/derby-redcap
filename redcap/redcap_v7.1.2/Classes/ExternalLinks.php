...
class ExternalLinks
{
	...
	public function displayresourcesTable()
	{
		...
		$table_html = renderGrid("resources", "", 1000, "auto", $resourcesTableHeaders, $resourcesTableData, true, false, false);
		// Return the html
		return $table_html;
	}
		...
		if (db_num_rows($q) > 0)
		{
			...
			while ($row = db_fetch_assoc($q))
			{
				...
				if (is_numeric($row['project_id'])) {
					...
					$extLinkImg = "<img src='".APP_PATH_IMAGES."application_link.png'>&nbsp;";
				}
		...
		if (defined('PROJECT_ID') && is_numeric(PROJECT_ID)) {
			...
			if (SUPER_USER) {
				$sql = "select distinct p.project_id, trim(p.app_title) as app_title
						...
						where a.username = u.username and u.project_id = " . PROJECT_ID . " and a.project_id = p.project_id and p.status in (0,1)
