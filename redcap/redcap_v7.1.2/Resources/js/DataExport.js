...
$(function(){
	...
	$('div#center h3, div#sub-nav, #showPlotsStatsOptions').addClass('hide_in_print');
...
function showExportFormatDialog(report_id, odm_export_metadata) {
	...
	$('#exportFormatDialog').dialog({ title: title, bgiframe: true, modal: true, width: 870, open: function(){ fitDialog(this) }, buttons:
		...
		{text: (odm_export_metadata ? exportBtnTxt2 : exportBtnTxt), click: function() {
			...
			exportajax = $.post(app_path_webroot+'DataExport/data_export_ajax.php?pid='+pid+getSelectedInstrumentList()+getInstrumentsListFromURL()+getLiveFilterUrl(), params, function(data) {
...
function loadReportNewPage(pagenum) {
	...
	if (getParameterByName('stats_charts') == '1') {
		// STATS (load new page)
		window.location.href = app_path_webroot+'DataExport/index.php?pid='+pid+'&stats_charts=1&report_id='+report_id+(report_id=='SELECTED' ? '&instruments='+getParameterByName('instruments') : '')+dynamicFiltersUrl;
	} else {
		...
		var newUrl = app_path_webroot+'DataExport/index.php?pid='+pid+'&report_id='+report_id+(report_id=='SELECTED' ? '&instruments='+getParameterByName('instruments') : '')+'&pagenum='+pagenum+dynamicFiltersUrl;
...
function openQuickAddDialog() {
	...
	$.post(app_path_webroot+'DataExport/report_quick_add_field_ajax.php?pid='+pid, { checked_fields: flds.join(',') }, function(data){
		...
		// Display success dialog
		simpleDialog(null, json_data.title, 'quickAddField_dialog', 600);
