...
function playAudioObject(ob,iconob) {
	...
	survey_obs.each(function(event){
		...
		if (ob.text() != "") {
			if (id == 'return_code_completed_survey_div' || id == 'codePopupReminderTextCode' || id == 'codePopupReminderText' || id == 'survey-login-instructions') {
				var btn = '<img src="'+app_path_images+'speaker.gif" class="'+icon_class+'" title="Speak the displayed text" '
						+ 'onclick="playAudioObject($(\'#'+id+'\'),this);">';
				ob.prepend(btn);
			} else if (isMobileDevice) {
				var btn = '<img src="'+app_path_images+'speaker.gif" class="'+icon_class+'" style="margin-right:5px;" title="Speak the displayed text" '
						+ 'onclick="playAudioObject($(\'#'+id+'\'),this);">';
				ob.children().eq(0).prepend(btn);
			} else {
				var btn = '<img src="'+app_path_images+'speaker.gif" class="'+icon_class+'" style="position:absolute;top:'+top+'px;left:'+left+'px;" title="Speak the displayed text" '
						+ 'onclick="playAudioObject($(\'#'+id+'\'),this);">';
				ob.append(btn);
			}
		}
	...
	$('.sliderlabels-ne, .survey-login-error-msg, #survey_auth_form table tr .labelrc, .popup-contents, #questiontable tr, #questiontable .note, '
	 +'#questiontable .fileuploadlink, #questiontable select.x-form-field, #questiontable .choicevert, #questiontable .choicehoriz').each(function(event){
		...
		if (isField) {
			...
			var btn = '<img src="'+app_path_images+'speaker.gif" class="'+icon_class+'" style="'+style+'" title="Speak the displayed text" '
					+ 'onclick="playAudioObject($(\'#'+id+'\'),this);">';
			if (isSectionHeader) {
