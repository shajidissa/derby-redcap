...
$(function(){
  ...
  $('.process-request-btn').on('click', function(){
    ...
    $('.todo-iframe').load(function () { //The function below executes once the iframe has finished loading
  ...
  $('.expand-btn').on('click', function(e){
    ...
    if($(this).hasClass('open')){
      ...
      $(this).closest('.request-container').velocity({height:140});
      setTimeout(function(){
        $(that).addClass('open');
      },100);
    }
  ...
  function writeComment(id, comment){
    ...
    function(data){
      if (data == '1'){
        $('.write-comment-dialog .confirm-btn').text('Success!');
        setTimeout(function(){
          location.reload();
        },1000);
      }
  ...
  function openWriteComment(id){
    ...
    commentText = (commentText === 'None' ? '' : commentText);
  	simpleDialog('Write or edit a comment for this request.','Comment',1,400);
  	var $input =  $('<textarea>',{
  		'class': 'comment-text',
      text: commentText
  	}),
    $erase =  $('<a>',{
  	...
  	$('body').find('.ui-dialog').addClass('write-comment-dialog').append($confirm);
  	$('.ui-dialog-content').append($input).append($erase);
  	$input.focus();
  	//bind confirm button click to Enter hit
  ...
  function updateCollapsibleSections(collapseInfo){
    ...
    for (var i = 0; i < $arrowUp.length; i++) {
      // console.log($($arrowUp[i]).attr('class'));
      // console.log(infoArray[i]);
      //change it only if need to be close
      if(infoArray[i] === 'close'){
          $($arrowUp[i]).addClass('hide').removeClass('show');
          $($arrowUp[i]).next().addClass('show').removeClass('hide');
  ...
  $('.pending-title .collapse-section-icon').on('click', function(e){
    ...
    if(childClass === 'collapse-up-arrow'){
      ...
      $(this).addClass('hide').removeClass('show');
      $(this).prev().addClass('show').removeClass('hide');
