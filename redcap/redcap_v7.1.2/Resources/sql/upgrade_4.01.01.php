			...
			while ($row2 = db_fetch_assoc($q2))
			{
				...
				if ($failed_regex)
				{
					...
					print "insert into redcap_log_event (project_id, ts, user, ip, page, event, object_type, sql_log, pk, event_id, data_values, description) values "
						. "($this_project_id, ".str_replace(array(" ","-",":"), array("","",""), NOW).", 'USERID', '".System::clientIpAddress()."', 'DataEntry/index.php', 'UPDATE', 'redcap_data', ".checkNull($sql).", '".prep($row2['record'])."', '{$row2['event_id']}', '$this_field_name = \'$this_date\'', 'Update record');\n";
				}
			}
		}
	}
