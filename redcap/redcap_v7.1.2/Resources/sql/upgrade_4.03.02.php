...
if (db_num_rows($q) > 0)
{
	...
	while ($row = db_fetch_assoc($q))
	{
		...
		print "insert into redcap_log_event (project_id, ts, user, ip, page, event, object_type, sql_log, pk, event_id, data_values, description) values "
			. "({$row['project_id']}, ".str_replace(array(" ","-",":"), array("","",""), NOW).", 'USERID', '".System::clientIpAddress()."', 'DataEntry/index.php', 'UPDATE', 'redcap_data', ".checkNull($sql).", '".prep($row['record'])."', '{$row['event_id']}', '{$row['field_name']} = \'" . prep($row['value']) . "\'', 'Update record');\n\n";
	}
}
db_free_result($q);