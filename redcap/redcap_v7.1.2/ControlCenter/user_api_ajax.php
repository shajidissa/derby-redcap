...
elseif ($_GET['action'] == 'reassignToken_s') {
	if (empty($_GET['api_username'])) {
		...
		if (isset($_GET['showDropDownOnly'])) {
			...
			$sql = "update redcap_user_information set api_token = null where username = '" . prep($_GET['api_username']) . "'";
			db_query($sql);
			// Reassign token to new user
			$sql = "update redcap_user_information set api_token = '" . prep($token) . "' where username = '" . prep($_GET['new_user']) . "'";
			if (!db_query($sql)) {
				$ajaxData = RCView::errorBox($lang['control_center_261'] . ' - ' . $lang['control_center_259'], 'dialogAJAXId');
			}
			else
