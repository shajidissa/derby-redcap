...
if (!defined("PROJECT_ID")) {
	...
	$html .= RCView::h4(array('style' => 'margin-top: 0;'),
				RCView::img(array('src'=>'coins.png')) .
...
$html .= RCView::h4(array('style' => 'margin-top: 35px;'),
			RCView::img(array('src'=>'coins_silver.png')) .
...
$(function()
{
	...
	function apiRightsCheck_s(url, urlAfterCheck) {
		...
		$.ajax({url: url, type: 'POST', data: { redcap_csrf_token: redcap_csrf_token },
			success: function(data) {
				$("#rightsDialogId").html(data);
				$("#rightsDialogId").attr('title', "<?php echo cleanHtml2($lang['control_center_4507']) ?> \"" + $("#rightsUsername").val() + "\"");
				RedCapUtil.closeLoader($("#apiDummyContainer").parent());
				$("#rightsDialogId").dialog({ bgiframe: true, modal: true, width: 550, close: function() {$(this).dialog('destroy');}, buttons: {
					Cancel: function() { $(this).dialog('close'); },
					'<?php echo cleanHtml($lang['control_center_4509']) ?>': function() {
						RedCapUtil.openLoader($("#apiDummyContainer").parent());
						$(this).dialog('close');
	...
	$(document).delegate("a[id^=sApiDelId]", "click", function() {
		if (confirm("<?php echo cleanHtml2($lang['control_center_4502']) ?>")) {
			RedCapUtil.openLoader($("#apiDummyContainer").parent());
			$.post($(this).attr("href"),{  },
				function (data) {
					redrawAPITables(data);
	...
	$(document).delegate("a[id^=sApiRegenId]", "click", function() {
		if (confirm("<?php echo cleanHtml2($lang['control_center_4501']) ?>")) {
	...
	$(document).delegate("a[id^=sApiViewId]", "click", function() {
		...
		var title1 = "<?php echo cleanHtml2($lang['control_center_4503']) ?>";
		var title2 = "<?php echo cleanHtml2($lang['control_center_4515']) ?>";
		initDialog("dialogAJAXId");
		$("#dialogAJAXId")
			.html("<?php echo cleanHtml2(RCView::b($lang['control_center_4504']).' '.$lang['control_center_4526'].RCView::br().RCView::br().RCView::b($lang['control_center_324'])) ?>")
	...
	$(document).delegate("a[id^=sApiReassignId]", "click", function() {
		var url = $(this).attr("href");
		var title1 = "<?php echo cleanHtml2($lang['control_center_4511']) ?>";
		...
		$.post(url+(url.indexOf('?')>0 ? '&' : '?')+'showDropDownOnly=1' ,{  },function(userOptions){
			...
			initDialog("dialogAJAXId");
			$("#dialogAJAXId")
				.html("<?php echo cleanHtml2(RCView::b($lang['control_center_4512']).' '.$lang['control_center_4517'].RCView::br().RCView::br()."<b>".$lang['control_center_4513'].' "<span id="reassign_user_old"></span>" '.$lang['data_access_groups_ajax_14']."</b>".RCView::select(array('id'=>'reassign_user', 'class'=>'x-form-text x-form-field', 'style'=>'margin-left:5px;'), array())) ?>")
				.addClass("simpleDialog");
	...
	$(document).delegate("a[id^=apiReassignId]", "click", function() {
		...
		$.post(url+(url.indexOf('?')>0 ? '&' : '?')+'showDropDownOnly=1',{ },function(userOptions){
			...
			initDialog("dialogAJAXId");
			$("#dialogAJAXId")
				.html("<?php echo cleanHtml2(RCView::b($lang['control_center_4444']).' '.$lang['control_center_4445'].RCView::br().RCView::br()."<b>".$lang['control_center_4448'].' "<span id="reassign_user_old"></span>" '.$lang['data_access_groups_ajax_14']."</b>".RCView::select(array('id'=>'reassign_user', 'class'=>'x-form-text x-form-field', 'style'=>'margin-left:5px;'), array())) ?>")
				.addClass("simpleDialog");
			$('#reassign_user').html(userOptions);
			$('#reassign_user_old').html(thisuser);
			// Remove the current user
