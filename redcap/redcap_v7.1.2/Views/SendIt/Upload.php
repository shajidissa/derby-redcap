	...
	function validateRecipients() {
		if (recipients.val().length == 0) {
			...
			for (var i=0; i<emailsArr.length; i++) {
				if (!isEmail(emailsArr[i])) badEmails += "<br>" + emailsArr[i];
			}
			if (badEmails == "") {
				return true;
			} else {
				simpleDialog('<?php echo cleanHtml($lang['sendit_07']."<br><br><b>".$lang['sendit_08']."</b>") ?>'+badEmails,'<?php echo cleanHtml($lang['global_01']) ?>','',400);
...
<?php
	...
	<form action="<?php echo $_SERVER['REQUEST_URI']."&loc=$fileLocation".($fileId == '' ? "" : "&id=$fileId") ?>" enctype="multipart/form-data" target="_self" method="post" id="form1" name="form1">
	<div id="senditbox">
		<fieldset>
			...
			<div style="margin-bottom: 10px; clear: both;">
				<label for="recipients" class="labelrc">
					...
					<textarea id="recipients" name="recipients" style="width:100%;max-width: 400px; height: 80px;"><?php echo (isset($recipients) && $recipients != '') ? htmlspecialchars($recipients, ENT_QUOTES) : '' ?></textarea>
			...
			<div style="color:#555; margin-bottom: 15px; clear: both;">
				<label for="message" class="labelrc" style="font-weight:normal;" ><?php echo $lang['sendit_27'] ?> <br><?php echo $lang['global_06'] ?></label>
					<textarea id="message" name="message" style="width:100%;max-width: 400px; height: 80px;"><?php echo (isset($message) && $message != '') ? htmlspecialchars($message, ENT_QUOTES) : '' ?></textarea>
			</div>
