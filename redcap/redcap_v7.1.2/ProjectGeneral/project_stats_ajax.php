...
if (isset($_GET['pid']) && is_numeric($_GET['pid']))
{
	...
	if ($last_logged_event == "") {
		...
		$sql = "select timestamp(ts) from redcap_log_event
	...
	$num_records = User::number_format_user(Records::getRecordCount());
	...
	if ($_POST['type'] == 'records')
	{
		...
		if (!empty($pids))
		{
			...
			while ($row = db_fetch_assoc($q))
			{
				...
				db_query("replace into redcap_record_counts (project_id, record_count) values ({$row['project_id']}, {$row['count']})");
			}
		}
	}
