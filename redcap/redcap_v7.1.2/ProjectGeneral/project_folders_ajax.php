...
if(isset($_GET['edit_folder']))
{
	...
	if($folder_id)
	{
		...
		?>
		simpleDialog('<?php echo cleanHtml($edit_folder); ?>', '<?php echo cleanHtml($lang['folders_10']); ?>', 'edit_folder_popup', 450, '', '<?php echo cleanHtml($lang['global_53']); ?>', 'saveFolder();', '<?php echo cleanHtml($lang['folders_11']); ?>');
...
// handle hide archived checkbox
if(isset($_POST['hide_archived']))
{
	$_SESSION['hide_archived'] = isset($_POST['archived']) ? (int)$_POST['archived'] : 0;
}
...
if(isset($_POST['new_folder']))
{
	...
	if(!strlen($name))
	{
		?>simpleDialog('<?php echo cleanHtml($lang['folders_14']); ?>');<?php
	}
	elseif($exists)
	{
		?>simpleDialog('<?php echo cleanHtml($lang['folders_15']); ?>');<?php
	}
	else
	{
		?>
		...
		if($folder_id)
		{
			?>editFolder(<?php echo $folder_id; ?>,1);<?php
		}
		?>
		$('#folderName').val('');
		<?php
