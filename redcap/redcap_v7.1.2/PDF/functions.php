...
function setFooterImage($pdf)
{
	...
	$pdf->Cell(85,0,'');
	$pdf->Cell(0,0,'www.projectredcap.org',0,0,'L',false,'https://projectredcap.org');
	//Set the REDCap logo
...
function qtext_vertical($row, $char_limit_q) {
	...
	if ($project_encoding != '') {
		...
		foreach ($lines as $key=>$line) {
			foreach (str_split($line, $char_limit_q) as $newline) {
				if ($newline == '') continue;
				$lines2[] = $newline;
			}
		}
	...
	// Go through each line and place \n to break up into segments based on $char_limit_q value
	foreach ($lines as $key=>$line) {
		$numbreaks = floor(strlen($line)/$char_limit_q);
		if ($numbreaks >= 1) {
			$start_pos = $char_limit_q;
...
function text_vertical($this_string,$char_limit) {
	...
	if ($project_encoding != '') {
		$lines2 = array();
		foreach ($lines as $key=>$line) {
			foreach (str_split($line, $char_limit) as $newline) {
				if ($newline == '') continue;
				$lines2[] = $newline;
			}
		}
...
function atext_vertical_mc($row, $dataNormalized, $char_limit_a, $indent_a, $project_language)
{
	global $project_encoding;
...
function removeSectionHeader($pdf)
{
	global $pdfLastSH;
...
function removeMatrixHeader($pdf)
{
	global $pdfLastMH;
			...
			foreach ($eventNormalized as $event_id=>$nattr1) 
			{
				...
				foreach ($nattr1 as $repeat_instrument=>$nattr2) 
				{
					foreach ($nattr2 as $repeat_instance=>$dataNormalized) 
					{
						...
						//print "ADDED - metadata count: ".count($this_metadata);
							...
							foreach ($this_metadata as $key=>$attr)
							{
								...
								foreach ($elements_attr_to_replace as $this_attr_type)
								{
									...
									if ($this_string != '')
									{
										...
										$this_metadata[$key][$this_attr_type] = strip_tags(Piping::replaceVariablesInLabel($this_string, $record, $event_id, $repeat_instance, $piping_record_data));
									}
								}
							}
						}
						...
						foreach ($this_metadata as $row)
						{
							...
							if ($last_form != $row['form_name']) // || $last_repeat_instrument != $repeat_instrument || $last_repeat_instance != $repeat_instance)
							{
								...
								if (isset($Proj) && is_array($ProjForms) && isset($ProjForms[$row['form_name']]['survey_id'])) {
									...
									$survey_instructions = strip_tags(Piping::replaceVariablesInLabel($survey_instructions, $record, $event_id, $repeat_instance, $piping_record_data));
								...
								//Initial page number
								$pdf->Cell(0,2,"Page ".$pdf->PageNo().($GLOBALS['project_encoding'] == 'chinese_utf8' ? '' : ' of {nb}'),0,1,'R');
								...
								if (isset($isSurvey) && $isSurvey)
								{
									...
									if ($atSurveyPkField && isset($response_time_text) && $response_time_text != "") {
										$pdf->SetFont(FONT,'',10);
							...
							if ($row['element_preceding_header'] != "" && $record != '' && $fieldsDisplayedInSection === 0 && $encounteredFirstSH && $row['field_order'] != '2') {
								$pdf = removeSectionHeader($pdf);
							}
							...
							if ($row['element_type'] == "yesno" || $row['element_type'] == "truefalse" || $row['element_type'] == "radio" || $row['element_type'] == "select" || $row['element_type'] == "advcheckbox" || $row['element_type'] == "checkbox" || $row['element_type'] == "sql")
							{
								...
								{
									$q_lines = qtext_vertical($row, $char_limit_q);
									$a_lines = atext_vertical_mc($row, $dataNormalized, $char_limit_a, $indent_a, $project_language);
									if ($row['element_note'] != "") {
										$a_lines_note = text_vertical($row['element_note'], $char_limit_a);
										foreach ($a_lines_note as $row2) {
											$a_lines[] = $row2;
							...
							// Text, Notes, Calcs, and File Upload fields
								...
								if ($row['custom_alignment'] == 'LV' || $row['custom_alignment'] == 'LH')
								{
									...
									if (isset($dataNormalized[$row['field_name']]) && $row['element_type'] == "file" && $row['element_validation_type'] == "signature") {
										...
										if (!USE_UTF8) {
											...
											$pdf = new_page_check(2, $pdf, $y_units_per_line, $bottom_of_page, $study_id_event);
											$pdf->Ln();
								...
								else
								{
									...
									if (isset($dataNormalized[$row['field_name']]) && $row['element_type'] == "file" && $row['element_validation_type'] == "signature") {
										...
										if ($row['element_note'] != '') {
											if (substr($row['element_note'], 0, 1) == "\n") $row['element_note'] = substr($row['element_note'], 1);
											$a_lines = text_vertical($row['element_note'], $char_limit_a);
						...
						if ($record != '')
						{
							...
							$sql = "select display, display_esignature, label from redcap_locking_labels where project_id = " . PROJECT_ID . "
									and form_name = '" . (isset($_GET['page']) ? $_GET['page'] : '') . "' limit 1";
							$q = db_query($sql);
							...
							if ($displayLocking)
							{
								...
								$sql = "select l.username, l.timestamp, u.user_firstname, u.user_lastname from redcap_locking_data l, redcap_user_information u
										...
										and l.record = '" . prep(isset($_GET['id']) ? $_GET['id'] : '') . "' and l.event_id = '" .
										prep(isset($_GET['event_id']) ? $_GET['event_id'] : '') . "'
										and l.form_name = '" . prep(isset($_GET['page']) ? $_GET['page'] : '') . "' limit 1";
								$q = db_query($sql);
								if (db_num_rows($q))
								{
