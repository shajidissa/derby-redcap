...
if (SUPER_USER && isset($_POST['bioportal_api_token']) && !empty($_POST['bioportal_api_token']))
{
	...
	if (empty($results)) {
		...
		// SUCCESS
		// Save the token
		$sql = "update redcap_config set value = '".prep($_POST['bioportal_api_token'])."' where field_name = 'bioportal_api_token'";
		db_query($sql);
		// Title
...
// Return JSON
print json_encode(array('success'=>($error_msg == '' ? '1' : '0'), 'content'=>$content, 'title'=>$title));