...
if (isset($_REQUEST['action']))
{
	switch ($_REQUEST['action'])
	{
		...
		case "delete":
			// Logging
			Logging::logEvent("","redcap_events_metadata","MANAGE",$_GET['event_id'],eventLogChange($_GET['event_id']),"Delete event");
		...
		case "add":
			if (is_numeric($_GET['day_offset']) && is_numeric($_GET['offset_min']) && is_numeric($_GET['offset_max'])) {
				...
				$sql = "insert into redcap_events_metadata (arm_id, day_offset, descrip, offset_min, offset_max, custom_event_label)
						values ($arm_id, '".prep($_GET['day_offset'])."', '".prep(label_decode($_GET['descrip']))."',
						'".prep($_GET['offset_min'])."', '".prep($_GET['offset_max'])."', ".checkNull(label_decode($_GET['custom_event_label'])).")";
				db_query($sql);
				...
				// Logging
				Logging::logEvent($sql,"redcap_events_metadata","MANAGE",$new_event_id,eventLogChange($new_event_id),"Create event");
				// Hidden value
		...
		case "edit":
			if (!$scheduling || ($scheduling && is_numeric($_POST['day_offset']) && is_numeric($_POST['offset_min']) && is_numeric($_POST['offset_max']))) {
				// Update the event
				if ($scheduling) {
					$sql = "update redcap_events_metadata set descrip = '".prep(label_decode($_POST['descrip']))."',
							day_offset = '".prep($_POST['day_offset'])."',
							offset_min = '".prep($_POST['offset_min'])."', offset_max = '".prep($_POST['offset_max'])."', 
							custom_event_label = ".checkNull(label_decode($_POST['custom_event_label']))."
							where event_id = '".prep($_POST['event_id'])."'";
				} else {
					$sql = "update redcap_events_metadata set descrip = '".prep(label_decode($_POST['descrip']))."', 
							custom_event_label = ".checkNull(label_decode($_POST['custom_event_label']))."
							where event_id = '".prep($_POST['event_id'])."'";
				...
				checkFirstEventChange($arm);
				// Logging
				Logging::logEvent($sql,"redcap_events_metadata","MANAGE",$_POST['event_id'],eventLogChange($_POST['event_id']),"Edit event");
			} else {
				//Give warning message if error exists
				print "<div class='red'><b>{$lang['global_01']}:</b> {$lang['define_events_35']}</div>";
			}
			...
			if (!$scheduling && isset($_POST['event_ids'])) {
				...
				foreach (explode(",", $_POST['event_ids']) as $this_event_id) {
					...
					$sql = "update redcap_events_metadata set day_offset = ".$day_offset++."
							where event_id = '".prep($this_event_id)."'";
					db_query($sql);
				}
				// CHECK IF FIRST EVENT CHANGED. IF SO, GIVING WARNING ABOUT THE PUBLIC SURVEY LINK CHANGING
				checkFirstEventChange($arm);
				// Logging
				Logging::logEvent($sql,"redcap_events_metadata","MANAGE","",eventLogChange($_POST['event_ids']),"Reorder events");
			}
			// Reload project events so that new unique event names are reflected
			$Proj->loadEvents();
			$Proj->loadEventsForms();
			...
			RCView::div(array('id'=>'downloadUploadEventsArmsDropdownDiv', 'style'=>'text-align:left;display:none;position:absolute;z-index:1000;'),
				RCView::ul(array('id'=>'downloadUploadEventsArmsDropdown'),
					...
					(!($super_user || $status < 1 || ($status > 0 && $enable_edit_prod_events)) ? '' :
						RCView::li(array(),
							RCView::a(array('href'=>'javascript:;', 'style'=>'color:#8A5502;', 'onclick'=>"simpleDialog(null,null,'importArmsDialog',500,null,'".cleanHtml($lang['calendar_popup_01'])."',\"$('#importArmsForm').submit();\",'".cleanHtml($lang['design_530'])."');$('.ui-dialog-buttonpane button:eq(1)',$('#importArmsDialog').parent()).css('font-weight','bold');"),
					...
					(!($super_user || $status < 1 || ($status > 0 && $enable_edit_prod_events)) ? '' :
						RCView::li(array(),
							RCView::a(array('href'=>'javascript:;', 'style'=>'color:#333;', 'onclick'=>"simpleDialog(null,null,'importEventsDialog',500,null,'".cleanHtml($lang['calendar_popup_01'])."',\"$('#importEventsForm').submit();\",'".cleanHtml($lang['design_530'])."');$('.ui-dialog-buttonpane button:eq(1)',$('#importEventsDialog').parent()).css('font-weight','bold');"),
...
if (!isset($arm_id) || $arm_id == "" || isset($_GET['action']) && $_GET['action'] == "rename_arm")
{
	...
	print  "<input type='text' size='25' maxlength='50' class='x-form-text x-form-field' id='arm_name' value='" . htmlspecialchars(label_decode($_GET['arm_name']), ENT_QUOTES) . "'> &nbsp;
			&nbsp; &nbsp; {$lang['define_events_40']} &nbsp;<input type='text' class='x-form-text x-form-field' style='width:20px;' maxlength='2' id='arm_num' value='$arm' onblur=\"redcap_validate(this,'1','99','soft_typed','int')\"> &nbsp;
			<br><br>
			<input type='button' value='".cleanHtml($lang['designate_forms_13'])."' style='font-size:11px;' id='savebtn' onclick=\"
				if (document.getElementById('arm_name').value.length > 0 && document.getElementById('arm_num').value.length > 0) {
					...
					simpleDialog('".cleanHtml($lang['define_events_41'])."');
				}
			\"> &nbsp;
			<input type='button' value='".cleanHtml($lang['global_53'])."' style='font-size:11px;' id='cancelbtn' onclick=\"
				this.disabled = true;
				document.getElementById('progress').style.visibility = 'visible';
				document.getElementById('arm_name').disabled = true;
				document.getElementById('arm_num').disabled = true;
	...
	print  "<table class='form_border' id='event_table' style='clear:both;width:100%;'>
			<tr class='nodrop'>" .
				...
				<td class='labelrc' style='text-align:center;background-color:#eee;font-size:11px;'>
					{$lang['design_671']}
					<a href='javascript:;' onclick=\"simpleDialog('".cleanHtml($lang['design_672'])."', '".cleanHtml($lang['design_671'])."');\"><img title=\"".cleanHtml2($lang['form_renderer_02'])."\" src='".APP_PATH_IMAGES."help.png'></a>
				...
				<td class='labelrc' style='font-weight:normal;text-align:center;background-color:#eee;font-size:10px;'>
					...
					<a href='javascript:;' onclick=\"simpleDialog('".cleanHtml($lang['define_events_67'])."', '".cleanHtml($lang['define_events_65'])."');\"><img title=\"".cleanHtml2($lang['form_renderer_02'])."\" src='".APP_PATH_IMAGES."help.png'></a><br>
	...
	//Visits have been defined, so display them
		...
		while ($row = db_fetch_assoc($q))
		{
			...
			if (isset($_GET['edit']) && $_GET['event_id'] == $row['event_id']) {
				...
				if (!$scheduling) {
					print  "<td class='data ".(($super_user || $status < 1 || ($status > 0 && $enable_edit_prod_events)) ? 'dragHandle' : '')."' style='text-align:center;width:20px;'></td>";
				}
				print "
					<td class='data' style='text-align:center;'>
						<input type='button' id='editbutton' value='".cleanHtml($lang['designate_forms_13'])."' style='font-size:11px;' onclick='editVisit($arm,{$row['event_id']})'>
					</td>
	...
	if ($super_user || $status < 1 || ($status > 0 && $enable_edit_prod_events))
	{
		print  "<tr class='nodrop'>" .
					...
					<td class='data' valign='top' colspan='2' style='text-align:center;background:#eee;padding:15px 10px 0px 0px;'>
						<input id='addbutton' type='button' value='".cleanHtml($lang['define_events_16'])."' onclick=\"addEvents($arm,$num_events_total);\">
					</td>" .
					(!$scheduling ? "" :
