				...
				<table border=0 cellspacing=0 width=100%>
					...
					<tr>
						...
						<td style='vertical-align:middle;padding-top:10px;color:#555;'>
							<input class='x-form-text x-form-field' style='width:50%;' type='text' id='emailTo' name='emailTo' onkeydown='if(event.keyCode == 13){return false;}' onblur=\"redcap_validate(this,'','','soft_typed','email')\" value='".cleanHtml($requestor_email)."'>
							&nbsp; {$lang['database_mods_142']}
						</td>
					</tr>
					<tr>
						<td style='vertical-align:middle;width:60px;padding:10px 0;font-weight:bold;'>{$lang['survey_103']}</td>
						<td style='vertical-align:middle;padding:10px 0;'><input class='x-form-text x-form-field' style='width:80%;' type='text' id='emailTitle' name='emailTitle' onkeydown='if(event.keyCode == 13){return false;}' value='".cleanHtml("[REDCap] ".$lang['database_mods_141'])."'></td>
					</tr>
					<tr>
...
?>
<div id="diff-display" title="<?php print cleanHtml2($lang['design_674']) ?>">
...
$(function() { 
	...
	$('#diff-display').dialog({
		...
		width: 750,
		buttons: {
			'<?php echo cleanHtml($lang['calendar_popup_01']) ?>': function() {
				$( this ).dialog( "close" );
			}
		},
		open: function(event, ui) {
			...
			for (var oldChoiceVal in oldChoices) {
				...
				if (newChoices.hasOwnProperty(oldChoiceVal)) {
					...
					status = (oldChoices[oldChoiceVal] === newChoices[oldChoiceVal])
								? '<?php echo cleanHtml($lang['design_680']) ?>' : '<div class="red"><?php echo cleanHtml($lang['design_681']) ?></div>';
					rowItems.push(status);
				} else {
					rowItems.push('-');
					rowItems.push('-');
					rowItems.push('<div class="red"><?php echo cleanHtml($lang['design_682']) ?></div>');
				}
			...
			for (var i=0; i < addedChoiceVals.length; i++) {
				...
				rowItems.push('<div class="darkgreen"><?php echo cleanHtml($lang['design_683']) ?></div>');
				rowItems.push('<span style="color:#aaa;">-</span>');
				trs.push(rowItems);
			}
...
var textChoicesToObject = function (text) {
	...
	for (var i=0; i < choices.length; i++) {
		var choice = choices[i].split(', ', 2);
		if (choice[1] != null) rtnObj[trim(choice[0])] = trim(choice[1]);
	}
	return ksort(rtnObj);
};
function ksort(obj){
