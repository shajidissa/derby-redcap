...
if ($answer != null && isset($_SESSION['field']) && $_SESSION['field'] != '' && isset($Proj->metadata[$_SESSION['field']]))
{
	...
	if ($Proj->metadata[$_SESSION['field']]['stop_actions'] != '' && $Proj->isMultipleChoice($_SESSION['field'])) {
		$stop_actions = parseStopActions($Proj->metadata[$_SESSION['field']]['stop_actions']);
		$endSurveyViaStopAction = in_array($answer, $stop_actions);
	}
	//db_query("insert into aaa (mytext) values ('".prep("field = {$_SESSION['field']}\nanswer = $answer\n".print_r($stop_actions, true))."')");
...
if ($field == null) {
	...
	if (!isset($_SESSION['field']) && $instructions != '') {
		if ($record_exists) {
			$instructions = Piping::replaceVariablesInLabel($instructions, $fetched, $event_id, 1, $record_data);
		}
	...
	if ($nextField !== false)
	{
			...
			do {
				...
				elseif ($isDescriptiveField) {
					...
					if ($Proj->metadata[$field]['element_preceding_header'] != '') {
						...
						if ($record_exists) {
							$element_preceding_header = Piping::replaceVariablesInLabel($element_preceding_header, $fetched, $event_id, 1, $record_data);
						}
						if (SMS) {
					...
					if ($record_exists) {
						$label = Piping::replaceVariablesInLabel($label, $fetched, $event_id, 1, $record_data);
					}
		...
		if ($field == $form_name."_complete" || $endSurveyViaStopAction)
		{
			...
			if ($end_survey_redirect_next_survey) {
				...
				$next_survey_url = Survey::getAutoContinueSurveyUrl($fetched, $form_name, $_GET['event_id']);
				...
				if (SMS) {
					...
					$_SESSION = array();
					$_SESSION['survey_access_code'] = Survey::getAccessCode(getParticipantIdFromHash($next_survey_hash), false, false, true);
					// Redirect
			...
			$acknowledgement = trim(replaceNBSP(strip_tags(Piping::replaceVariablesInLabel(label_decode($Proj->surveys[$survey_id]['acknowledgement']), $fetched, $event_id))));
			if ($acknowledgement != '') {
				if (SMS) {
