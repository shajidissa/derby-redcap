...
if (checkSurveyProject($_GET['survey_id']) && isset($_GET['hash']) && isset($_GET['custom_url']))
{
  if(isset($_GET['arm_id'])){
    $arm_id = $_GET['arm_id'];
    $sql = "select * from redcap_events_arms where project_id = $project_id and arm_id = '".prep($arm_id)."'";
    $q = db_query($sql);
	...
	if ($shorturl === false) exit('0');
	if (isset($shorturl['errorCode']) && $shorturl['errorCode'] != 0) exit("".$shorturl['errorCode']);
	// save to db logic
	if($custom_public_survey_links != '' && is_array(json_decode($custom_public_survey_links, true))) {
		...
		foreach ($result as &$value) {
		  if($arm_num == $value->{'arm_number'}){
			$value->{'custom_url'} = $shorturl['shortURL'];
			$newCustomurlString = "]";
		  }else{
			$newCustomurlString = ',{"arm_number":"'.$arm_num.'", "custom_url":"'.$shorturl['shortURL'].'"}]';
		  }
		}
		$jsonString = json_encode($result);
		$jsonString = str_replace(']', $newCustomurlString, $jsonString);
	} else {
		$jsonString = '[{"arm_number":"'.$arm_num.'", "custom_url":"'.$shorturl['shortURL'].'"}]';
	}
	$sql = "update redcap_projects set custom_public_survey_links='".prep($jsonString)."' where project_id = $project_id";
	$q = db_query($sql);
    //send data to front-end
    echo 	RCView::div(array('style'=>'font-size:15px;'), 
				$lang['control_center_4568'] . 
				RCView::a(array('href'=>$shorturl['shortURL'], 'style'=>'font-weight: bold; letter-spacing: 1px;display:block;font-size:16px;line-height: 24px;text-decoration:underline;', 'target'=>'_blank'), $shorturl['shortURL']) .
