...
if ($_POST['action'] == 'popup')
{
	...
	if (!empty($emailToDropdownOptions)) {
		$emailToDropdown =  RCView::select(array('class'=>'x-form-text x-form-field','style'=>' margin-bottom:5px;','id'=>'followupSurvEmailToDD','onchange'=>'inviteFollowupSurveyPopupSelectEmail(this);'),
				...
				RCView::div(array('style'=>'padding:10px 0 10px 2px;'),
					RCView::table(array('cellspacing'=>'0','border'=>'0','width'=>'100%'),
						...
						RCView::tr(array('id'=>'compose_email_subject_tr'),
							...
							'<input class="x-form-text x-form-field" style="width:280px;" type="text" id="followupSurvEmailSubject" onkeydown="if(event.keyCode == 13){return false;}" value="'.cleanHtml2(str_replace('"', '&quot;', label_decode(isset($emailSubject) ? $emailSubject : ''))).'"/>'
							)
						) .
						// Message
						RCView::tr(array('id'=>'compose_email_form_fieldset'),
							RCView::td(array('colspan'=>'2','style'=>'padding:5px 0 10px;'),
								'<div class="textarea-preview-parent" message="#followupSurvEmailMsg" subject="#followupSurvEmailSubject" from="#followupSurvEmailFrom">
								...
								<textarea class="x-form-field notesbox" id="followupSurvEmailMsg" style="height:100px;width:95%;">'.nl2br(label_decode(isset($emailContent) ? $emailContent : '')).'</textarea>' .
								// Extra instructions
...
elseif ($_POST['action'] == 'email' && isset($_POST['email']))
{
	...
	$content = Piping::replaceVariablesInLabel($content, $_POST['record'], $_GET['event_id'], 1, array(), true, null, false);
	...
	foreach ($participantSendTimes as $reminder_num=>$thisSendTime) {
		$sql = "insert into redcap_surveys_scheduler_queue (email_recip_id, record, scheduled_time_to_send, reminder_num, instance)
				values ($email_recip_id, ".checkNull($_POST['record']).", '".prep($thisSendTime)."', '".prep($reminder_num)."', '".prep($_GET['instance'])."')";
