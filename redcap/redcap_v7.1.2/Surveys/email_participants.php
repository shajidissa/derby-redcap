...
else
{
	...
	foreach ($participant_records as $this_part=>$this_record)
	{
		...
		// Set content
		$this_content = ($doPipingContent) ? Piping::replaceVariablesInLabel($_POST['emailCont'], $this_record, $_GET['event_id'], 1, array(), true, null, false) : $_POST['emailCont'];
	...
	if (db_query($sql)) {
		...
		foreach ($participantSendTimes as $reminder_num=>$thisSendTime) {
			...
			$sql = "insert into redcap_surveys_scheduler_queue (email_recip_id, record, scheduled_time_to_send, reminder_num, instance)
					values ($email_recip_id, ".checkNull($this_record).", '".prep($thisSendTime)."', '".prep($reminder_num)."', $instance)";
